...
-- Feature test.
--
-- This exists because vim.fn.has() returns 0 or 1, and 0 is truthy in Lua.
-- Every plugin in this file used to be guarded by a bare `vim.fn.has(...)`,
-- which meant none of those version gates did anything at all.
...
--
-- Copilot is the only one actually installed. The rest keep their full
-- configuration below so they can be turned back on by flipping a flag here,
-- but their specs are never handed to lazy.nvim, so nothing is downloaded,
-- built or loaded for them.
--
-- avante additionally runs `make` at install time, and nvim-aider expects the
-- aider CLI on $PATH - see doc/ubuntu-setup.md if you enable either.
local aiPlugins = {
	copilot = true,
	codeium = false,
	aider   = false,
...
-- ChatGPT also needs a key file to be present.
local openaiKeyFile = vim.fn.expand('$HOME/.openai-api-key')
local enableChatGPT = aiPlugins.chatgpt and vim.fn.filereadable(openaiKeyFile) == 1

-- Codeium config
if aiPlugins.codeium then
...
-- CodeiumAI
if has('nvim-0.6.0') and aiPlugins.codeium then
	table.insert(pluginspec, { 'Exafunction/codeium.vim' })
end
-- CoPilot
if has('nvim-0.6.0') and aiPlugins.copilot then
...
-- Aider
if has('nvim-0.9.4') and aiPlugins.aider then
	table.insert(pluginspec, {
...
-- Avante
if has('nvim-0.10.1') and aiPlugins.avante then
	table.insert(pluginspec, {
		'yetone/avante.nvim',
		event = 'VeryLazy',
		version = false,
		...
		opts = {
			...
			mappings = {
				...
				-- <leader>vh is already bound to toggle.hint above; this was a
				-- duplicate, so history selection moved to <leader>vH.
